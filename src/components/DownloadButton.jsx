...
export default function DownloadButton({
  ...
  // Position calculation: By default opens DOWNWARDS so the top is never hidden behind the navbar.
  // Only opens upwards if there isn't enough space below the viewport edge.
  useEffect(() => {
  ...
  return (
    <div className={`relative inline-flex items-center ${isOpen ? "z-50" : ""}`}>
      ...
      {/* Quality Options Dropdown Popup Menu */}
      {isOpen && (
        <div
          ref={dropdownRef}
          ...
          } z-[9999] w-56 sm:w-60 rounded-xl bg-surface-container-high/95 backdrop-blur-2xl border border-white/15 shadow-[0_16px_40px_rgba(0,0,0,0.9)] p-2 flex flex-col gap-1 animate-in fade-in zoom-in-95 duration-150 select-none text-left`}
        >
          {/* Header */}
          ...
          {/* 4 Quality Options (MB size hidden per request) */}
            ...
            {QUALITY_OPTIONS.map((opt) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 min-w-0 flex-1">
                  <div className={`w-6 h-6 rounded-md flex items-center justify-center bg-white/5 border border-white/5 group-hover:scale-105 transition-transform flex-shrink-0 ${opt.iconColor}`}>
                  ...
                  <div className="flex items-center gap-1.5 min-w-0">
                    ...
                    <span className={`px-1.5 py-0.2 rounded text-[8.5px] font-mono font-bold border ${opt.badgeClass}`}>
                      {opt.tag}
                    </span>
                  </div>
                </div>
